...
$pass_signin = $_POST['pass_signin']; //  @md5($_POST['pass_signin']);
...
if ($db->db_num_rows($sql_check_user_exist))
{
    ...
    $sql_check_login = "SELECT username, password FROM users WHERE username = '$user_signin' AND password = '$pass_signin'";
 
    // Nếu đúng
    if ($db->db_num_rows($sql_check_login))
    {
    ...
    {
        echo $show_alert.'Mật khẩu không chính xác, hãy kiểm tra lại.';
    }
}
