...
{   
    ...
    if ($ac == 'create')
    {
        ...
        $title_create_note = $db->db_real_escape_string(@$_POST['title_create_note']);
        $body_create_note = $db->db_real_escape_string(@$_POST['body_create_note']);
...
{
    // Nhận dữ liệu và gán vào các biến đồng thời xử lý chuỗi
    $title_edit_note = $db->db_real_escape_string(@$_POST['title_edit_note']);
    $body_edit_note = $db->db_real_escape_string(@$_POST['body_edit_note']);
    $id_edit_note = $db->db_real_escape_string(@$_POST['id_edit_note']);
    ...
    // Nếu có
    if ($db->db_num_rows($sql_check_id_exist))
    {
...
else if ($ac == 'delete')
{
    ...
    $sql_check_id_exist = "SELECT id_note, user_id FROM notes WHERE user_id = '$data_user[user_id]' AND id_note = '$id_edit_note'";
 
    // Nếu có
    if ($db->db_num_rows($sql_check_id_exist))
    {
        // Lệnh SQL xoá note
        $sql_delete_note = "DELETE FROM notes WHERE user_id = '$data_user[user_id]' AND id_note = '$id_edit_note'";
        // Thực thi truy vấn
        $db->db_query($sql_delete_note);
        // Giải phóng kết nối
        $db->db_close();
