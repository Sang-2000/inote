<div class="container">
    <div class="row">
        <div class="col-md-12">
            <h3 class="text-primary">Danh sách các note (Tí xóa: khi hover vào các note sẽ hiện lên ghi chú của note)</h3>
            <div class="list-group">
                ...
                // Nếu có 
                if ($db->db_num_rows($sql_get_data_list_note))
                {
                    // In danh sách ghi chú
                    foreach ($db->db_fetch_assoc($sql_get_data_list_note, 0) as $key => $data_list_note) {
                        ...
                        echo '
                            <div class="col-md-6">
                                <a href="index.php?ac=edit_note&id='.$data_list_note['id_note'].'" class="list-group-item ">
                                    <div>
                                        <h4 class="list-group-item-heading" style="color: red;"><strong>'.$data_list_note['title'].'</strong></h4>
                                        <hr/>
                                    </div>
                                    <p class="list-group-item-text">'.$data_list_note['body'].'</p>
                                    <small style="color: salmon;"> Tạo ngày
                                        '.$day_created.' tháng
                                        '.$month_created.' năm
                                        '.$year_created.' lúc
